...
const ToolsGrid = ({ tools, tag }: ToolsGridProps) => {
    ...
    return (
        <div className="w-full relative gap-x-7 gap-y-11 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 card:grid-cols-3 
            bg-primary-bg px-3 lg:px-8 py-12 rounded-3xl lg:rounded-tl-none lg:rounded-tr-none">
            {
                machineState === 'loading' ? (
                    <p className='w-full text-center text-gray-500 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>Loading...</p>
                ) : machineState === 'idle' ? (
