...
import updateToolMachine from '../machines/tool-machines/UpdateToolMachine';
import type { ToolCategory, ToolTag, NewTool, Tool } from '../types';
...
const UpdateToolPage = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const tool = location.state?.tool as Tool | undefined;
  const { send: sendTools } = useTools();
  const [state, send] = useMachine(updateToolMachine);
  ...
  useEffect(() => {
    if (tool) {
      ...
      // If no tool provided, redirect back
      navigate(-1);
    }
  }, [tool, send, navigate]);
  ...
  useEffect(() => {
    if (state.matches('success') && state.context.toolResponse) {
      const updatedTool = state.context.toolResponse;
     
      // Update tool in ToolMachine
      sendTools({
        type: 'UPDATE_TOOL',
        tool: updatedTool,
      });
      
      // Navigate to tool view page after a brief delay to show success state
      setTimeout(() => {
        navigate(`/tool/${updatedTool._id}`);
      }, 1000);
    }
  }, [state, sendTools, navigate]);
  ...
  return (
    <section className="flex flex-col">
      ...
      >
        Update the details of your tool.
      </p>
      ...
      >
        ...
        >
          {isLoading ? 'Updating...' : isSuccess ? 'Updated!' : 'Update Tool'}
        </button>
      </form>
    </section>
  );
